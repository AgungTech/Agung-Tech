...
class agung.VideoPlaybackLight {
	...
	public var $resizeMode:Number = 2;
	...
	public function load(url:String) {
		...
		netStr.onMetaData 	= UtilsAdjusted.delegate(this, $onMetaData);
		netStr.onStatus 	= UtilsAdjusted.delegate(this, $onStatus);
	...
	public function setMaxSize(pW:Number, pH:Number) {
		maxW 				= pW;
		maxH				= pH;
	}
	...
	private function scaleVideo() {
		...
		vidOb._width = Math.round(cw);
		vidOb._height = Math.round(ch);
		...
		if ($resizeMode == 1) {
			...
			vidOb._x = Math.round(mw / 2 - cw / 2);
			vidOb._y = Math.round(mh / 2 - ch / 2);
		}
		else {
			obj.w = vidOb._width;
			obj.h = vidOb._height;
