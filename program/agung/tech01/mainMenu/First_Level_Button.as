 ...
 */
class agung.tech01.mainMenu.First_Level_Button extends MovieClip 
{
	...
	public function setNode(pNode:XMLNode, pSettingsObj:Object) {
		...
		if (node.attributes.type == "shop") {
			_global.shopsArray.push(this);
		}
		
		if (node.attributes.type == "cart") {
			_global.successCart = node.attributes.successModule
			_global.cartArray.push(this);
		}
	...
	public function dispatchMc() {
		if (!subMenu) {
			...
			_global.parentTitleLevelOne = urlTitle;
			_global.parentAddressLevelOne = urlAddress;
	...
	public function ponRelease() {
		if (!subMenu && (_global.moduleLoadingState == 0)) {
			if (node.attributes.externalLink == 1) {
