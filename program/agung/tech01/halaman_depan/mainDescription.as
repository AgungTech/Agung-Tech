...
class agung.tech01.halaman_depan.mainDescription extends MovieClip
{
 ...
 * @param	pSettings
 */
	public function setNode(pNode:XMLNode, pMainSettings:Object, pSettingsScrollerBox:Object, pSettings:Object) {
		node = pNode;
		settingsObj = pSettings;
		scrollerBoxSettingsObj = pSettingsScrollerBox;
		mainSettingsObj = pMainSettings;
		
		if (scrollerBoxSettingsObj.enableScrollerBox == 0) {
			totalWidth = pMainSettings.moduleWidth;
		}
		else {
			...
			totalWidth = Math.round(pMainSettings.moduleWidth - scrollerBoxTotalWidth);
		}
		
		lst["txt"]._width = totalWidth;
		mask._width = totalWidth;
		mask._height = settingsObj.descriptionHeight;
