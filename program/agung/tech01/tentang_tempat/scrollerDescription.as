...
/**
 * this class handles the tooltip you can see
 * when mousing over the images on the scroller
 * */
class agung.tech01.tentang_tempat.scrollerDescription extends MovieClip
{
	...
	private function onMouseMove() {
		if (tsSettings.toggleTooltip == 1) {
			...
			if (tsSettings.enableTooltipAutoCorrectX == 0) {
				var newX:Number = Math.ceil(this._parent._xmouse - newBgWidth + tsSettings.tooltipXCorrect - 10);
			}
			else {
	...
	public function setNewText(pNode:XMLNode) {
		if (tsSettings.toggleTooltip == 1) {
			...
			Tweener.addTween(mask, { _width:newBgWidth, _height:lastText["txt"].textHeight + 2, time:.4, transition:"easeOutQuad" } );
