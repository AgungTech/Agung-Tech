...
/**
 * This class handles the small item from the sharing list
 */

class agung.tech01.main_l.shareSmallItem extends MovieClip
{
	...
	public function shareSmallItem() {
	...
	/**
	 * this function receives the node and loads the proper image into the item
	 * @param	pNode
	 */
	public function setNode(pNode:XMLNode) {
		...
		hit._width = settingsObj.smallButtonsWidth;
		hit._height = settingsObj.smallButtonsHeight + 15;
		hit._alpha = 0;
	...
	private function onLoadInit(mc:MovieClip) {
		getImage(mc, true);
		var o:Object = getDims("fit", mc._width, mc._height, settingsObj.smallButtonsWidth, settingsObj.smallButtonsHeight, false);
		mc._width = o.w;
		mc._height = o.h;
		mc._x = o.x;
		mc._y = origY = 0;
	...
	private function hitOnRollOver() {
		Tweener.addTween(mcc, { _y:origY - 3, time: .3, transition: "easeOutBack" } );
		dispatchEvent( { target:this, type:"rolledOverSmall", mc:this } );
	}
	
	private function hitOnRollOut() {
		Tweener.addTween(mcc, { _y:origY, time: .2, transition: "linear" } );
		dispatchEvent( { target:this, type:"rolledOutSmall", mc:this } );
	}
