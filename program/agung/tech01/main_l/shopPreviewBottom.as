...
/**
 * This class handles the bottom shop preview
 */
class agung.tech01.main_l.shopPreviewBottom extends MovieClip
{
	private var hit:MovieClip;
	
	private var handlers:MovieClip;
	private var handlersMask:MovieClip;
	...
	public function shopPreviewBottom() {
		this._alpha = 0;
		
		handler = handlers["handler"];
		handlerEmpty = handlers["handlerEmpty"];
		
		handlersMask._visible = false
		...
		actualIcon["over"]["txt"].autoSize = true;
		actualIcon["over"]["txt"].wordWrap = false;
		
		actualIcon["normal"]["normal"]["txt"].autoSize = true;
		actualIcon["normal"]["normal"]["txt"].wordWrap = false;
	...
	public function killPanelOnBottom() {
			permitRollOut = 1;
			iconOnRolOut();
			clearInterval(myInterval);
			hideCart();
	...
	private function scanCart() {
		if (emptyState == 0) {
			if ((hit._xmouse > 0) && (hit._xmouse < hit._width) && (hit._ymouse > 0) && (hit._ymouse < hit._height + 6)) {
				//trace("ok");
				trace(hit._ymouse + " " + hit._height)
			}
			else {
	...
	public function init() {
		...
		actualIcon["over"]["txt"].text = actualIcon["normal"]["normal"]["txt"].text = _global.cartSettings.bottomCartButtonCaption;
		actualIcon["over"]["bg"]._width = icon["hit"]._width = actualIcon["normal"]["normal"]["bg"]._width = Math.ceil(actualIcon["normal"]["normal"]["txt"].textWidth + 8 + 8 + 8 + 13);
		
		
		
		
		hit._x = handler._x = handlerEmpty._x = defXHandler = Math.ceil(-handler._width + 180);
		hit._y = handler._y = defYHandler = Math.ceil( -handler._height - 2);
		
		handlerEmpty._y = Math.ceil( -handlerEmpty._height);
		
		handlerEmpty._y = handlerEmpty._height;
		handler._y = handler._height;
		
		handlersMask._x = hit._x;
		handlersMask._y = hit._y;
		
		handlers.setMask(handlersMask);
	...
	private function openCart() {
		...
		clearInterval(myInterval);
		Tweener.addTween(handler, { _y:defYHandler, time:.5, transition:"easeOutQuad" } );
		Tweener.addTween(handlerEmpty, { _y:Math.ceil( -handlerEmpty._height + 3), time:.5, transition:"easeOutQuad" } );
		
		
		_global.mp3player.forceClosePlayList();
	}
	
	private function hideCart() {
		Tweener.addTween(handler, { _y:handler._height, time:.5, transition:"easeOutQuad", onComplete:Proxy.create(this, invisHandler) } );
		Tweener.addTween(handlerEmpty, { _y:Math.ceil( handlerEmpty._height), time:.5, transition:"easeOutQuad", onComplete:Proxy.create(this, invisHandlerEmpty) } );
	}
	...
	public function updateItems() {
		...
		for (idx = 1; idx <= 4; idx++) {
			if (myArr[myArr.length - idx]) {
				var currentItem:MovieClip = lst.attachMovie("IDshopPreviewItemBottom", "shopPreviewItem" + idx, lst.getNextHighestDepth());
